...
SearchMenu = 0;
while SearchMenu ~= 6
    SearchMenu = menu('Building Inventory Database','Enter Data',...
        'Display Table','Total Area','Average Capacity','Query','Quit');
    switch SearchMenu
        case 1
...
%problem 6: Query option
%prompts user fo a field name and a condition
%outputs the number of rooms in the database that meet the
%specified condition
FieldName = input('Enter the name of a field in the database: ','s');
Quantity = input('Enter a number: ');
%check all conditions in all the fields
%write an rmquery function